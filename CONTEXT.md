...
**What it represents:** The decision surface. This is where the complexity of GURU Intelligence is distilled into clear, human-readable decisions for the advisor. It is warm, editorial, and confident — like a private banking brief or a morning memo from a trusted CFO. The advisor layer never overwhelms; it presents one clear action with supporting context.

**Visual language:**
- Background: `#f0ece5` (linen) — warm, off-white, never pure white
- Cards: `#ffffff` with subtle `border: 1px solid rgba(0,0,0,0.08)`
...
- Numbers in this layer: Inter or JetBrains Mono at smaller scale, navy (`hsl(222,45%,12%)`)
...
- Tone: editorial, calm, advisory, like a letter from a trusted advisor

**Reference screen:** "How GURU organizes your money" bucket card layout — colored top border per category, clean white card, Playfair headline, warm grid. This is the canonical Advisor Layer visual.
...
### Colors
- `--linen: #f0ece5` — Advisor Layer background
...
- ~$5M+ net worth, HNW
- 3-month reserve floor: ~$90K operating cash
...
- ❌ No abbreviations anywhere in the UI — write out full names always. "Money Market Fund" not "MMF", "Treasury" not "TSY", "basis points" not "bps" in copy, etc. This applies to account names, labels, descriptions, and data rows.
